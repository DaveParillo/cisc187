...
{
    list<double>::iterator p = find_if_range(v.begin(), v.end(), Larger_than(31));
    if (p!=v.end()) { /* we found value > 31 */ }

    list<double>::iterator q = find_if_range(v.begin(), v.end(), Larger_than(x));
...
int main()
{
    ...
    find_if_range(v.begin(),v.end(),Larger_than(31));
}
