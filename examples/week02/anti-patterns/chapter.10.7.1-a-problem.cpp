...
//
// This code shows how not to prompt a user for input
// Problem: the entire program is in 1 function.
  ...
  try
  {
    ...
    while (true)    {
      ...
      if (cin) {    // we got an integer; now check it:
        ...
        if (!cin) {
          throw std::runtime_error("no input");     // we didn't find a digit: give up
        }
