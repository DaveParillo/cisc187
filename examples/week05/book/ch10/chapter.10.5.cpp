...
try
{
    ...
    if (!ist) throw std::runtime_error("can't open input file "+name);

    cout << "Please enter name of output file: ";
    cin >> name;
    ofstream ost(name.c_str());    // ost writes to a file named "name"
    if (!ost) throw std::runtime_error("can't open output file "+name);

    vector<Reading> temps;    // store the readings here
    int hour;
    double temperature;
    while (ist >> hour >> temperature) {
        if (hour < 0 || 23 <hour) throw std::runtime_error("hour out of range");
