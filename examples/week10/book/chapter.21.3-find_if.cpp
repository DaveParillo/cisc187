...
In find_if_range(In first, In last, Pred pred)
{
...
void f(vector<int>& v)
{
    vector<int>::iterator p = find_if_range(v.begin(), v.end(), odd);
...
{
    list<double>::iterator p = find_if_range(v.begin(), v.end(), larger_than_42);
...
void f(list<double>& v, int x)
{
    ...
    list<double>::iterator p = find_if_range(v.begin(), v.end(), larger_than_v);
    if (p!=v.end()) { /* we found value > 31 */ }

    ::v = x;  // set v to x for the next call of larger_than_v
    list<double>::iterator q = find_if_range(v.begin(), v.end(), larger_than_v);
